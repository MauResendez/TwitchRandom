...
export default function Home() {
  ...
  const checkViewersInput = (e) => {
    if (e.target.value < 10 && e.target.value != '') // If the viewer amount is set less than 10, then set it to 10
    {
      setViewers(10)
      // sessionStorage.setItem('viewers', String(viewers));
    }
    else if (e.target.value > 9999999 && e.target.value != '') // If the viewer amount is set higher than 9999999, then set it to 9999999 (Max Amount)
    {
      setViewers(null)
      // sessionStorage.setItem('viewers', null);

    }
    else if (e.target.value >= 10 && e.target.value != '') // If the viewer count is equal or more than 10, set the viewers to the number
    {
      ...
      <Container maxWidth="sm" sx={{ textAlign: "center" }}>
        ...
        <FormControl sx={{ width: '100%', my: 1 }} onSubmit={findRandomStream}>
          <Autocomplete
            ...
            options={games}
            freeSolo
            renderInput={(params) => <TextField {...params} variant='outlined' id='game' label='Game' name='game' autoFocus />}
          />
          <TextField sx={{ mb: 2 }} 
            variant='outlined' 
            type='number' 
        ...
        <Box sx={{ display: 'flex', justifyContent: 'center', my: 2 }}>
          <Button type='submit' variant='contained' color='primary' sx={{ mr: 3 }} onClick={() => { router.push('/stream') }}>Find A Random Stream</Button>
          <Button type='button' variant='contained' color='primary' sx={{ ml: 3 }} onClick={() => { router.push('/results') }}>View Results</Button>
        </Box>
        <Box sx={{ my: 5 }}>
          <Copyright />
        </Box>
