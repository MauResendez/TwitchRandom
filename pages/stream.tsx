...
const TwitchStream = dynamic(() => import("react-twitch-embed-video"), {
  ssr: false,
});

export default function Stream() {
  const [{ stream, loading, error }, setState] = useState({
    stream: null,
    loading: true,
    error: null,
  });
  ...
  async function findStream(game: string, viewers: number, language: string) {
    ...
    try {
      if (channel) {
        // If you clicked on a stream from the Results page, it'll set up the stream to be that one
        setState({ stream: channel, loading: false, ...error });
        sessionStorage.removeItem("channel");
      } else if (!(results && results.length > 0)) {
        // If you first started searching, it'll retrieve results and pick a random stream from there
        ...
        setState({
          stream: response.data[randomNumber].user_login,
          loading: false,
          ...error,
        });
      } else {
        // If you already have results, it'll go back to the results array and choose another stream from there
        ...
        do {
          randomNumber = Math.floor(
            Math.random() * JSON.parse(sessionStorage.getItem("results")).length
          );
          ...
          isLiveAndOnGame = await axios.get(
            `/api/stream?game_name=${game}&user_login=${results[randomNumber].user_login}`
          );
        ...
        setState({
          stream: JSON.parse(sessionStorage.getItem("results"))[randomNumber]
            .user_login,
          loading: false,
          ...error,
        });
  ...
  useEffect(() => {
    ...
    if (loading) {
      // If loading state is true (When you're first starting searching)
        ...
        {!loading && (
          <Box sx={{ display: "flex", justifyContent: "center", my: 2 }}>
            <Button
              ...
              className="submit"
              onClick={() => {
                setState({ ...stream, loading: true, ...error });
              }}
            >
              Find Another Stream
            </Button>
